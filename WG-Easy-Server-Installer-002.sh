...
Примеры:
  ...
  bash master.sh --password 'Qwerty123!' --country RU --refresh-endpoints
...
Параметры:
  ...
  --refresh-endpoints  Обновить WG_HOST и перескачать конфиги существующих клиентов
EOF
}
...
if [[ "$REFRESH_ENDPOINTS" == "1" ]]; then
  ...
  download_all_clients "$JAR" "$OUTDIR"
  log "Endpoint'ы обновлены. Новые конфиги сохранены в: $OUTDIR"
  echo "WG UI: http://${WG_HOST}:51821"
  exit 0
fi
